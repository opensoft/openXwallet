...
because that program (`scripts/validate-openxwallet.py`, `check_register`) lives
in this repository and runs inside the REQUIRED `wallet-validation` check of
every repository that pins openXwallet.

It is a NEW capability rather than a restatement of the intake's requirements:
the intake capability is not promoted here and never will be, and a delta
against a capability this repository cannot host would archive into a spec file
that does not exist. Nothing here re-opens an intake requirement. The intake's
own first-shape requirement already says per-seat scope is "a named successor
rather than part of the first shape" — this capability IS that successor, landing
beside that sentence rather than amending it.
